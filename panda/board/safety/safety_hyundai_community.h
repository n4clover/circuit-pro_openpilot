...
static int hyundai_community_rx_hook(CAN_FIFOMailBox_TypeDef *to_push) {
  ...
  if (valid) {
    ...
    if (addr == 914 && bus == HKG_mdps_bus) { // Read MDPS11, CR_Mdps_DrvTq : Driver Torque
      int driver_torque = ((GET_BYTES_04(to_push) & 0x7ff) * 0.79) - 808;
    }
...
static int hyundai_community_tx_hook(CAN_FIFOMailBox_TypeDef *to_send) {
  ...
  if (addr == 912) { // SPAS Steering Rate Limit Check
    int driver_torque = driver_torque
...
static int hyundai_community_fwd_hook(int bus_num, CAN_FIFOMailBox_TypeDef *to_fwd) {
  ...
  if ((HKG_forward_bus1 || HKG_forward_obd) && (addr == 688 ||addr == 608 || addr == 1136 || addr == 870 || addr == 881 || addr == 790)){fwd_to_bus1 = 1;}
