...
from selfdrive.car.hyundai.hyundaican import create_fca11, create_lkas11, create_clu11, \
  create_acc_opt, create_frt_radar_opt,\
...
STEER_MAX_OFFSET = 75 # How far from MAX LKAS torque to engage Dynamic SPAS when under 60mph.
###### SPAS #######
...
class CarController():
  ...
  def update(self, c, enabled, CS, frame, CC, actuators, pcm_cancel_cmd, visual_alert,
    ...
    if pcm_cancel_cmd and self.longcontrol and self.pcm_cnt == 0 and CS.out.cruiseState.enabled and not CS.CP.radarDisable or self.longcontrol and not CS.CP.radarOffCan: #Make SCC cancel when op disengage or last accel is kept (IDK) -JPR
