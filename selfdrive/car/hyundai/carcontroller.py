...
class CarController():
  ...
  def update(self, c, enabled, CS, frame, CC, actuators, pcm_cancel_cmd, visual_alert,
    ...
    # RSPA Controller. - JPR
    self.spas_rspa_controller.RSPA_Controller(c, CS, frame, self.packer, self.car_fingerprint, can_sends, apply_accel, stopping)
    ...
    if self.longcontrol and (CS.cruiseState_enabled or CS.CP.radarDisable or CS.CP.radarOffCan):
      if frame % 2 == 0:
        ...
        can_sends.extend(create_acc_commands(self.packer, enabled, apply_accel, jerk, int(frame / 2), self.lead_visible, CS.lead_distance, set_speed, stopping, self.gapsetting, CS.out.gasPressed, CS.CP.radarDisable, CS.has_scc14)) 
