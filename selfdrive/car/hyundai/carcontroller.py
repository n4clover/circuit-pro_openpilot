...
class CarController():
  def __init__(self, dbc_name, CP, VM):
    ...
    if CP.spasEnabled:
      ...
      self.ratelimit = 1.0 # Starting point - JPR
  ...
  def update(self, enabled, CS, frame, CC, actuators, pcm_cancel_cmd, visual_alert,
    ...
    if CS.spas_enabled:
      ...
      if apply_diff > 1.5 and enabled: # Rate limit for when steering angle is not apply_angle - JPR
        ...
        self.ratelimit = 1.0 # Reset it back to 0.5 - JPR
      ...
      spas_active = CS.spas_enabled and enabled and (CS.out.vEgo < SPAS_SWITCH or apply_diff > 3.5 and CS.out.vEgo < 26.82 and self.dynamicSpas and not CS.out.steeringPressed or abs(apply_angle) > 3.5)
    ...
    if pcm_cancel_cmd and (self.longcontrol and not self.mad_mode_enabled) or not enabled and self.enabled_cnt >= 1 and CS.out.cruiseState.enabled: #Make SCC cancle when op disengage or last accel is kept (IDK) -JPR
